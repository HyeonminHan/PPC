...
vector<PPC> make_sequenced_Plen_PC(vector<PointCloud<PointXYZRGB>::Ptr> pointclouds,
	...
	vector<int> camera_order(total_num_cameras);
	
	int mid = total_num_cameras / 2.0;
	camera_order[0] = mid;

	for (int i = 0; i <= total_num_cameras*2; i+=2) {
		int temp;
		if (i == 0) continue;

		camera_order[i-1] = mid - i / 2;
		if(total_num_cameras % 2 !=0 || i!=total_num_cameras*2)
			camera_order[i] = mid + i / 2;
	}

	for(int i = 0; i< total_num_cameras; i++)
		cout << camera_order[i] << endl;
	/*if (!mode) camera_order = { 4, 3, 5, 2, 6, 1, 7, 0 };
	else if(mode == hotelroom_r2_front_sample) camera_or
	...
	for (int i = 1; i < total_num_cameras; i++) {
			...
			for (int u = 0; u < _width; u++) {
				...
				if (confirm_img.at<int>(v, u) == -1) {
					...
					if (!mode) {
						...
						Z = depth_level_2_Z_s_direct(depth_img.at<ushort>(v, u));
					}
					else {
				...
				else
				{
					...
					if (!mode) {
						...
						Z_ = depth_level_2_Z(d[0]);
					}
					else if (mode == hotelroom_r2_front_sample) {
						Z_ = depth_level_2_Z_s_direct(depth_img.at<ushort>(v, u));
					}
					else {
						d_s = depth_img.at<Vec3s>(v, u);
						Z_ = depth_level_2_Z_s(d_s[0]);
					}
		...
		cout << i << "number : " << Plen_PC.size() << endl;
	...
	vector<Mat> depth_imgs)
{
	...
	for (int point_idx = 0; point_idx < firstPC->points.size(); point_idx++) {
		...
		point_ppc_1.color[camera_order[0]][0] = (short)firstPC->points[point_idx].r;
		point_ppc_1.color[camera_order[0]][1] = (short)firstPC->points[point_idx].g;
		point_ppc_1.color[camera_order[0]][2] = (short)firstPC->points[point_idx].b;
		point_ppc_1.occlusion_pattern[camera_order[0]] = true;
	...
	for (int i = 1; i < total_num_cameras; i++) {
		...
		for (int point_idx = 0; point_idx < Plen_PC.size(); point_idx++) {
			double X = Plen_PC[point_idx].geometry[0];
			double Y = Plen_PC[point_idx].geometry[1];
			double Z = Plen_PC[point_idx].geometry[2];
		...
		for (int v = 0; v < _height; v++) {
			for (int u = 0; u < _width; u++) {
				...
				else
				{
					...
					int sub_V = color[2] - Plen_PC[point_idx].color[nearCamNum][0];
					int sub_U = color[1] - Plen_PC[point_idx].color[nearCamNum][1];

					float sub_Z = Z - Plen_PC[point_idx].geometry[2];
					...
					if (abs(sub_U) < c_threshold && abs(sub_V) < c_threshold) {
						...
						point_ppc_3.geometry[0] = X;
						point_ppc_3.geometry[1] = Y;
						point_ppc_3.geometry[2] = Z;
						point_ppc_3.color[cam][0] = (short)color[2];
						point_ppc_3.color[cam][1] = (short)color[1];
						point_ppc_3.color[cam][2] = (short)color[0];
						point_ppc_3.occlusion_pattern[cam] = true;
...
void save_ppc(vector<PPC> ppc, string filename) {
	...
	for (vector<PPC>::iterator vit = ppc.begin(), vend = ppc.end(); vit != vend; vit++) {
		...
		fout.write((char*)&vit->geometry[0], sizeof(float));
		fout.write((char*)&vit->geometry[1], sizeof(float));
		fout.write((char*)&vit->geometry[2], sizeof(float));

		for (int i = 0; i < total_num_cameras; i++) {
			fout.write((char*)&vit->color[i][0], sizeof(short));
			fout.write((char*)&vit->color[i][1], sizeof(short));
			fout.write((char*)&vit->color[i][2], sizeof(short));
		}

		for (int i = 0; i < total_num_cameras; i++) {
			fout.write((char*)&vit->occlusion_pattern[i], sizeof(bool));
		}
	}
	fout.close();
}
...
vector<PPC> load_ppc(string filename) {
	...
	while (!fin.eof()) {
		...
		fin.read((char*)(&pc.geometry[0]), sizeof(float));
		fin.read((char*)(&pc.geometry[1]), sizeof(float));
		fin.read((char*)(&pc.geometry[2]), sizeof(float));

		for (int i = 0; i < total_num_cameras; i++) {
			fin.read((char*)&pc.color[i][0], sizeof(short));
			fin.read((char*)&pc.color[i][1], sizeof(short));
			fin.read((char*)&pc.color[i][2], sizeof(short));
		}
		for (int i = 0; i < total_num_cameras; i++) {
			fin.read((char*)&pc.occlusion_pattern[i], sizeof(bool));
		}
