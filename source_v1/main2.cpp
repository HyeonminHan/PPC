			...
			for (shape_of_voxel = 2; shape_of_voxel <= 2; shape_of_voxel++) {
				for (voxel_mode = 2; voxel_mode <= 2; voxel_mode++) {
					...
					for (int frame = 0; frame < 3; frame++)
					{
						...
						view_PC_yuvTorgb(registered_PC);

						int ref = total_num_cameras / 2;
						PointCloud<PointXYZRGB>::Ptr firstPC = make_PC(ref, color_imgs[ref], depth_imgs[ref]);
						...
						for (color_model_mode = 0; color_model_mode <= 0; color_model_mode++)
						{
							...
							else if (ppc_mode == 2)
							{
								...
								//Plen_PC = make_sequenced_Plen_PC(pointclouds, color_imgs, depth_imgs);
								Plen_PC = make_sequenced_Plen_PC(firstPC, color_imgs, depth_imgs);
						...
						//		<< (float)denormalization_s(1, geo_min[2], geo_max[2]) << endl;
							...
							string filename = "binaryFiles_V1/hotel/hotel_v1_f" + to_string(frame) + ".bin";
							cout << " Plen_PC_S.size()" << Plen_PC.size() << endl;
							...
							cout << "projection_PPC_with_hole_filling done ... " << endl;
							view_PC_yuvTorgb(pointclouds[4]);
