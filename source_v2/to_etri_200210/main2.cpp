...
int main()
{
	...
	else if(data_mode == hotelroom_r2_front_sample) load_file_name_mode4(color_names, depth_names);
			...
			for (shape_of_voxel = 2; shape_of_voxel <= 2; shape_of_voxel++) {
				for (voxel_mode = 3; voxel_mode <= 3; voxel_mode++) {
					...
					for (int frame = 0; frame < frame_num; frame++)
					{
						...
						{
							cout << "ppcmode : " << ppc_mode << " shape of voxel : " << shape_of_voxel << " voxel_mode : " << voxel_mode << " color model data_mode :" << color_model_mode << endl;
#endif
							cout << "make_PPC : ";
							clock_t t5 = clock();
							...
							if (data_mode == 0) filename = "binaryFiles_V" + version_ + "/ballet/ballet_v" + version_ + "_f" + to_string(frame) + ".bin";
							else if (data_mode == 1) filename = "binaryFiles_V" + version_ + "/fencing/fencing_v" + version_ + "_f" + to_string(frame) + ".bin";
							else if (data_mode == 2) filename = "binaryFiles_V" + version_ + "/intel/intel_v" + version_ + "_f" + to_string(frame) + ".bin";
							else if (data_mode == 3) filename = "binaryFiles_V" + version_ + "/tech/tech_v" + version_ + "_f" + to_string(frame) + ".bin";
							else if (data_mode == 4) filename = "binaryFiles_V" + version_ + "/hotel/hotel_v" + version_ + "_f" + to_string(frame) + ".bin";
							...
							projection_PPC_with_hole_filling(Plen_PC, projection_imgs, filled_imgs, pointclouds_, nNeighbor, window_size);//Plen_PC
							//projection_PPC_with_hole_filling(vec_ppc_temp, projection_imgs, filled_imgs, pointclouds_, nNeighbor, window_size);//Plen_PC_S
							clock_t t8 = clock();
							cout << float(t8 - t7) / CLOCKS_PER_SEC << endl;

							view_PC_yuvTorgb(pointclouds_[0]);
