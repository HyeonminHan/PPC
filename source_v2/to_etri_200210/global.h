...
public:
	...
	virtual Matrix3Xf GetCoef() {	return  Array3f::Zero();	}
	virtual void SetColor(uchar avrv, uchar avru, Matrix3Xf coeff, vector<bool> occ) {}
};
...
public:
	...
	uchar GetY(int idx) {
		uchar y;
		y = uchar(coef(0, 0) + coef(1, 0) * idx + coef(2, 0) * idx * idx);
	...
	Vec3b GetColor(int idx) {
		...
		//cout << "V : " << vuy[0] << " U : " << vuy[1] << " Y : " << vuy[2] << endl;
		return vuy;
	}
};
...
public:
	...
	uchar GetY(int idx) {
		...
		//if (coef(0, 0)  > 255) { //coef(0, 0) + coef(1, 0) * idx + coef(2, 0) * idx * idx > 255
		//	cout << "idx : " << idx << " / " << coef(0, 0) << " " << coef(1, 0) << " " << coef(2, 0) << endl;
		//	//cout << (int)y << " " << coef(0, 0) + coef(1, 0) * idx + coef(2, 0) * idx * idx << endl;
		//}
		return y;
	}
	...
	void SetColor(uchar avrv, uchar avru, Matrix3Xf coeff, vector<bool> occ) {
		avrV = avrv;
		avrU = avru;
		coef = coeff;
		occlusion = occ;
