	...
	if (camera_order[0] == 0) {
		for (int camera = 0; camera < total_num_cameras; camera++)
		{
			...
			if (data_mode == MSR3DVideo_Ballet) {
				...
				color_img = imread(path + "/col/" + color_names[camera][frame]);
				depth_img = imread(path + "/dep/" + depth_names[camera][frame], IMREAD_ANYDEPTH);
			}
	...
	for (int cam_num = 0; cam_num < total_num_cameras; cam_num++)
	{
		...
		for (int v = 0; v < _height; v++)
			for (int u = 0; u < _width; u++) {
				...
				if(is_hole_filled_imgs[cam_num].at<bool>(v, u))
	...
	int nNeighbor)
{
	...
	for (int i = 0; i < img.cols; i++) {
		for (int j = 0; j < img.rows; j++) {
			...
			is_hole_img.at<bool>(j, i) = is_hole;
		}
	}
...
void YUV_dev2(vector<PPC*> PPC, vector<vector<float>>& dev_pointnum, vector<float>& point_num_per_color)
{
	for (int point_num = 0; point_num < PPC.size(); point_num++) {
		float avr_y = 0, avr_u = 0, avr_v = 0;
		float avr_y_2 = 0, avr_u_2 = 0, avr_v_2 = 0;
	...
	for (int cam = 0; cam < total_num_cameras; cam++) {
		for (int i = 0; i < 4; i++) {
			dev_pointnum[cam][i] = dev_pointnum[cam][i] / (float)point_num_per_color[cam];
		}
	}
	cout << "sibal2" << endl;
}
